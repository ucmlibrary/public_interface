FACET_TYPES = [
    ('type_ss', 'Type of Item'),
    ('facet_decade', 'Decade'),
    ('repository_data', 'Contributing Institution'),
    ('collection_data', 'Collection'),
]


CAMPUS_LIST = [
    {'id': '1', 'name': 'UC Berkeley', 'slug': 'UCB'},
    {'id': '2', 'name': 'UC Davis', 'slug': 'UCD'},
    {'id': '3', 'name': 'UC Irvine', 'slug': 'UCI'},
    {'id': '10', 'name': 'UCLA', 'slug': 'UCLA'},
    {'id': '4', 'name': 'UC Merced', 'slug': 'UCM'},
    {'id': '5', 'name': 'UC Riverside', 'slug': 'UCR'},
    {'id': '6', 'name': 'UC San Diego', 'slug': 'UCSD'},
    {'id': '7', 'name': 'UC San Francisco', 'slug': 'UCSF'},
    {'id': '8', 'name': 'UC Santa Barbara', 'slug': 'UCSB'},
    {'id': '9', 'name': 'UC Santa Cruz', 'slug': 'UCSC'},
]
